...
class Webservice extends CI_Controller{
  ...
  public function __construct(){
    ...
    define("SITE_URL",'https://lamavietech.ml/ghaseeli_laundry/');
...
//print_r($soap->reply[0]['merchantReferenceCode']); 
    ...
    if ($soap->reply->decision =='ACCEPT') {
          ...
          $this->Webservice_model->insert_data('payment', $arr_data111);
        
          $this->Webservice_model->update_data("place_order",['payment_status'=>'Paid'],['id'=>$arr_data111['order_id']]); 
...
/*************  get_service *************/
 ...
 public function get_service()
	{
	  	$subcategoryList = $this->Webservice_model->get_all('sub_category');
     	if ($subcategoryList)
		{
 ...
 public function get_all_child_category()
  ...
  /*********** search_sub_category function****************/
   ...
   public function add_to_cart(){
           ...
           //  $arr_get = "user_id = '".$arr_data['user_id']."' AND item_id = '".$arr_data['item_id']."'  AND restaurant_id = '".$arr_data['restaurant_id']."' AND status = 'Pending'";
             ...
             if($add_to_cart1){
                  ...
                  // $arr_get = "user_id = '".$arr_data['user_id']."' AND item_id = '".$arr_data['item_id']."'  AND restaurant_id = '".$arr_data['restaurant_id']."'";
                        $arrr_get = ['quantity'=>$quantity];
                        $this->Webservice_model->update_data('add_to_cart',$arrr_get,$arr_get);
                         
                        $log = $this->Webservice_model->get_where('add_to_cart',$arr_get); 
                        //$cart = $this->db->query("SELECT * FROM add_to_cart WHERE user_id ='$user_id' AND restaurant_id='$restaurants_id'")->result_array();
                      //  $cart = $this->Webservice_model->get_where('add_to_cart',['user_id'=>$user_id,'branch_id'=>$branch_id]); 
             ...
             elseif($user_id == $add_to_cart[0]['user_id'] AND $branch_id != $add_to_cart[0]['branch_id']){
                ...
                die;  
                    ...
                    $id = $this->Webservice_model->insert_data('add_to_cart',$arr_data);
                        ...
                        $log = $this->Webservice_model->get_where('add_to_cart',$arr_gets);   
                       // $cart = $this->db->query("SELECT * FROM add_to_cart WHERE user_id ='$user_id' AND restaurant_id='$restaurants_id'")->result_array();
                          //$cart = $this->Webservice_model->get_where('add_to_cart',['user_id'=>$user_id,'restaurant_id'=>$restaurants_id]); 
                        $ressult['result']='add to cart successfull';
                        $ressult['message']='successfull';
                        $ressult['status']='1';
                        $json = $ressult;  
 ...
 public function add_placeorder(){
              ...
              $invoiceorder=  (rand(10,100000000));
               ...
               $pickup_lon = $this->input->get_post('pickup_lon');
                ...
                $sub_branch = $this->Webservice_model->get_where('admin',['sub_admin_id'=>$product[0]['branch_id']]);
