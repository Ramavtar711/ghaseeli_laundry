      ...
      <div class="col">
       ...
       <form action="" class="form-horizontal" enctype="multipart/form-data" id="" method="post" name="">
                     ...
                     <!--
                          <div class="form-group">
                          <label class="form-control-label" for="role">Select Service</label> 
                          <select class="form-control select2"  multiple="multiple" name="service_id[]">
...
function changeCode(service) {
 ...
 //    alert(service);
      $.ajax({
          ...
          {
              document.getElementById("code").value = dataHTML.vPhoneCode; 
          }
      });
  }
  
  function setSubcate(id,selected)
   {
       ...
       var request = $.ajax({
          type: "POST",
          url: '<?= base_url(); ?>admin/get_subcate',
          data: {id: id, selected: selected},
          success: function (dataHtml)
          {
             $("#sub_category_id").html(dataHtml);
           
          }
        }); 
   }
   
    setSubcate('<?=$row[service_id]?>','<?=$row[category_id]?>');
