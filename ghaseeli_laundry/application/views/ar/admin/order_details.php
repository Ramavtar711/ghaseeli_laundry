  ...
  <?php include('include/header_bar.php');
      ...
      <div class="header-body">
        <div class="row align-items-center py-4 pb-7">
          <div class="col-lg-6 col-7">
            <h4 class="h2 text-white d-inline-block mb-0">تفاصيل الطلبات</h4>
            <nav aria-label="breadcrumb" class="d-none d-md-inline-block">
              <ol class="breadcrumb breadcrumb-links breadcrumb-dark">
                ...
                <li class="breadcrumb-item active text-white" aria-current="page"> تفاصيل الطلبات </li>
              </ol>
            </nav>
          </div>
        </div>
    ...
    <div class="container-fluid mt--6">
        ...
        <div class="row">
            ...
            <i class="fa fa-print" aria-hidden="true"></i> فاتورة طباعة
                 </a>
                 </div>
            </div>
            <div class="col-md-3">
	  ...
	  <option value="">حدد مندوب التوصيل</option>
...
<div  style="" >
  <select class="form-control" name="payment_status" onchange="addpaymentOrderstatus('<?php echo $place_order[0]['id']?>',this.value)" style="border: 1px solid #ddd;     color: #525f7f;width: 98px;">
	    <option <?php if($place_order[0]['payment_status'] == 'Paid'){echo 'selected' ; } ?> value="Paid" >مدفوع</option>
        <option <?php if($place_order[0]['payment_status'] == 'Unpaid'){echo 'selected' ; } ?> value="Unpaid" >غير مدفوعة</option>
...
<div class="dropdown-menu-right">
 ...
 <select class="form-control" name="status" onchange="addOrderstatus('<?php echo $place_order[0]['id']?>',this.value)" style="border: 1px solid #ddd;     color: #525f7f;width: 98px;">
	        <option <?php if($place_order[0]['status'] == 'Pending'){echo 'selected' ; } ?> value="Pending" >قيد الانتظار</option>
            <option <?php if($place_order[0]['status'] == 'Confirmed'){echo 'selected' ; } ?> value="Confirmed" >مؤكد</option>
            <option <?php if($place_order[0]['status'] == 'Pickup'){echo 'selected' ; } ?> value="Pickup" >يلتقط</option>
            <option <?php if($place_order[0]['status'] == 'Progress'){echo 'selected' ; } ?> value="Progress" >تقدم</option>
            <option <?php if($place_order[0]['status'] == 'Shipped'){echo 'selected' ; } ?> value="Shipped" >شحنها</option>
            <option <?php if($place_order[0]['status'] == 'Delivered'){echo 'selected' ; } ?> value="Delivered" >تم التوصيل</option>
            <option <?php if($place_order[0]['status'] == 'Cancel'){echo 'selected' ; } ?> value="Cancel" >يلغي</option>
	</select>
</div>
                    ...
                    <div class="card-header" style="display: flex;">
                        <h3 class="mb-0">تفاصيل الطلبات</h3>
                    <a href="<?php echo base_url('admin/page/edit_order1/'.$order_id); ?>">
                     <button class=" btn btn-primary  p-0 m-0  " style="margin-left: 20px !important;padding: 0px 10px !important;">يضيف</button></a>
                    </div>
			...
			<div class="card mb-3 mb-lg-5">
				<div class="card-header" style="display: block!important;">
					<div class="row">
                        ...
                        <h4 style="color: #8a8a8a;">اختر وقت التاريخ :</h4><p><?= $place_order[0]['pickup_date']?> <?= $place_order[0]['pickup_time']?></p>
                        <h4 style="color: #8a8a8a;">موعد التسليم :</h4><p> <?= $place_order[0]['delivery_date']?> <?= $place_order[0]['delivery_time']?></p>
                        <h4 style="color: #8a8a8a;">اختر العنوان :</h4><p>  <?= $place_order[0]['pickup_address']?> </p>
						</div>
						...
						<div class="col-6 pt-2">
							<div class="text-right">
							    ...
							    <?php if($place_order[0]['payment_type']=='Card'){ $payment_type = 'بطاقة';}else{$payment_type = 'نقدي';}
							         
							          if($place_order[0]['payment_status']=='Paid'){ $payment_status = 'مدفوع';}else{ $payment_status = 'غير مدفوعة';}
							    
							  	if($place_order[0]['status'] =='Pending'){$status = 'قيد الانتظار' ;}
							  	elseif ($place_order[0]['status'] =='Confirmed') { $status = 'مؤكد';}
							    elseif ($place_order[0]['status'] =='Pickup') {$status = 'يلتقط';}
							    elseif ($place_order[0]['status'] =='Progress') {$status = 'تقدم';}
							   elseif ($place_order[0]['status'] =='Shipped') {$status = 'شحنها';}
							   elseif ($place_order[0]['status'] =='Delivered') {$status = 'تم التوصيل';}
							   elseif ($place_order[0]['status'] =='Cancel') {$status = 'يلغي';}
					        ...
					        ?>
							    ...
							    <h4 class="text-capitalize" style="color: #8a8a8a;">حالة السداد :<?= $payment_status;?></h4>
								<h4 class="text-capitalize" style="color: #8a8a8a;">نوع الدفع :<?= $payment_type;?></h4>
							<!--	<h4 class="" style="color: #8a8a8a;">Reference Code : <button class="btn btn-outline-primary btn-sm" data-target=".bd-example-modal-sm" data-toggle="modal">Add</button></h4>-->
								<h4 class="text-capitalize" style="color: #8a8a8a;">حالة الطلب : <label class="badge badge-soft-primary" style="font-size: 14px"><?= $status;?></label></h4>
							</div>
						</div>
					</div>
				</div>
						...
						<div class="media-body">
							...
							<div class="row">
								<div class="col-md-6 mb-3 mb-md-0">
									<strong><?= $product[0]['child_cate_name_ar']?></strong><br>
										<div class="font-size-sm text-body">
										<span>سعر :</span> <span class="font-weight-bold"><?= $product[0]['amount'];?> Tax(<?= $product[0]['tax']?>)</span>
									</div>
									<strong><u>الإضافات :</u></strong>
									<div class="font-size-sm text-body">
					...
					<div class="row justify-content-md-end mb-3">
						<div class="col-md-9 col-lg-8">
							<dl class="row text-sm-right">
								<dt class="col-sm-6">سعر الأصناف:</dt>
							...
							<!--	<dt class="col-sm-6">Tax / Vat:</dt>
							  ...
							  <!--  <dd class="col-sm-6"><?= $place_order[0]['tax']?></dd>-->
								<dt class="col-sm-6">تكلفة الملحق:</dt>
								<dd class="col-sm-6">
								<?= $extra_options_total;?>
									<hr>
								</dd>
								<dt class="col-sm-6">المجموع الفرعي:</dt>
								<dd class="col-sm-6">
								    
							   $ <?=  $sub_total =   $total + $extra_options_total  ;?></dd>
								     
								<dt class="col-sm-6">خصم القسيمة:</dt>
								<dd class="col-sm-6"><?= $place_order[0]['coupon_amount']?></dd>
								<dt class="col-sm-6">رسوم التوصيل:</dt>
								<dd class="col-sm-6">
					           	<?= $place_order[0]['delivery_charge']?>
									<hr>
								</dd>
								<dt class="col-sm-6">المجموع:</dt>
		...
		<div class="col-lg-4">
			<div class="card">
				<div class="card-header">
					<h4 class="card-header-title">عميل</h4>
				</div>
				<div class="card-body">
					...
					<div class="media align-items-center">
						...
						<div class="media-body">
							<span class="text-body text-hover-primary"><?= count($order_count);?> ترتيب</span>
						</div>
					...
					<hr>
					<div class="d-flex justify-content-between align-items-center">
						<h5>عنوان التسليم</h5><a class="link" data-target="#shipping-address-modal" data-toggle="modal" href="javascript:"></a>
